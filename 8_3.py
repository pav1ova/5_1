...
students_python = Student.select().join(Student_courses).where(Student_courses.course_id == 1)
for r in students_python:
	print(r.name, r.surname)


students_spb = Student.select().join(Student_courses).where(Student_courses.course_id == 1, Student.city == 'Spb')
